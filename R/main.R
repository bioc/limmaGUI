...
ReadImageProcessingFiles <- function(){
	...
	Try(if (!exists("BCMethodDefault",envir=limmaGUIenvironment))
	{
	...
	#)
	#Try(NEOffsetDefault <- get("NEOffsetDefault",envir=limmaGUIenvironment))
	Try(if (!exists("NEOffsetDefault",envir=limmaGUIenvironment))
	{
...
#        return()
	...
	#
	onCancel <- function() {
		Try(tkgrab.release(ttGALandTargetsandSpotTypes));Try(tkdestroy(ttGALandTargetsandSpotTypes));Try(tkfocus(.limmaGUIglobals$ttMain));Try(Abort<<-1)}
	#
...
OpenALimmaFile <- function(FileName){
	...
	Try(NEOffsetDefault <- get("NEOffsetDefault",envir=limmaGUIenvironment))
	Try(if (!exists("NEOffset",envir=limmaGUIenvironment))
	{
...
AboutLimmaGUI <- function(){
	Try(tkmessageBox(title="About limmaGUI",
	                 message=paste("This is limmaGUI Version ",getPackageVersion("limmaGUI"),
	                 ", using limma Version ",
